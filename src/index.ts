...
import { EnrageUIReader, EnragePosition } from "./scripts/EnrageUIReader";
...
var imgs = a1lib.webpackImages({
	zamorakChest: require("./images/zamorakChest.data.png"),
...
export async function initOnLoad() {
	if (window.alt1) {
		...
		alt1.overLayTextEx("Initializing ZamorakLogger...", a1lib.mixColor(255, 144, 0), 20, Math.round(alt1.rsWidth / 2), 200, 50000, "", true, true);
	}
...
export async function init() {
	...
	// ~ 11/21/2022
	let history = JSON.parse(localStorage.getItem("ZamorakLogger/History"))
	if(history != null){
	...
	if (window.alt1) {
		alt1.overLayClearGroup("overlays");
		alt1.overLaySetGroup("overlays");
		alt1.overLayTextEx("ZamorakLogger ready!", a1lib.mixColor(100, 255, 100), 20, Math.round(alt1.rsWidth / 2), 200, 2000, "", true, true);
	}
...
export async function cleardb(choice: any) {
	...
	if (choice == 1) { // Nuclear reset all
		if (window.alt1) {
			alt1.overLayClearGroup("overlays");
			alt1.overLaySetGroup("overlays");
			alt1.overLayTextEx("Resetting ZamorakLogger...", a1lib.mixColor(255, 144, 0), 20, Math.round(alt1.rsWidth / 2), 200, 4000, "", true, true);
		}
		...
		if (window.alt1) {
			alt1.overLayClearGroup("overlays");
			alt1.overLaySetGroup("overlays");
			alt1.overLayTextEx("ZamorakLogger successfully reset! Restarting...", a1lib.mixColor(100, 255, 100), 20, Math.round(alt1.rsWidth / 2), 200, 4000, "", true, true);
		}
	...
	if (localStorage.getItem("ZamorakLogger/multiButtonPressDetect") === "true") {
		if (!autobool) {
			await new Promise(resolve => setTimeout(function () {
				(document.getElementById("docapturebutton") as HTMLDivElement).setAttribute("onclick", "TestApp.capture(false)");
...
async function findtrailComplete(img: a1lib.ImgRef, autobool: boolean) {
	...
	try {
		...
		alt1.overLayRect(a1lib.mixColor(255, 144, 0), xRect, yRect, imgs.zamorakChest.width + 345, imgs.zamorakChest.height + 291, 60000, 2);
				...
				if (itemResults[itemResults.length - 1] !== "Blank") {
					...
					if (autobool) {
						try {
							...
							let lsHistory = JSON.parse(localStorage.getItem("ZamorakLogger/History"))[JSON.parse(localStorage.getItem("ZamorakLogger/History")).length-1][0];
							if (seeConsoleLogs) console.log("Checking arrays for equivalence:",JSON.parse(localStorage.getItem("ZamorakLogger/History"))[JSON.parse(localStorage.getItem("ZamorakLogger/History")).length-1][0], itemResultsNoBlanks);
					...
					let lagDetectValue = new ZamorakReader();
...
async function compareItems(item: ImageData) {
	...
	let found = [];
	if (localStorage.getItem("ZamorakLogger/Algorithm") == "resemblejs") {
		found = matches[0];
		const promises = [];
	...
	else if (localStorage.getItem("ZamorakLogger/Algorithm") == "pixelmatch") {
		...
		*/
	...
	else if (localStorage.getItem("ZamorakLogger/Algorithm") == "hybrid") {
		...
		let precision = parseFloat(localStorage.getItem("ZamorakLogger/hybridPrecision")); //1 does nothing
		await Promise.all(promises);
...
async function addHistoryToLs(value: number, items: any, quants: any, reward: any) {
	...
	let previous = [itemsArr, quants, value, reward, localStorage.getItem("ZamorakLogger/Count"), localStorage.getItem("ZamorakLogger/PrimaryKeyHistory"), currentDateTime];
	let temp = JSON.parse(localStorage.getItem("ZamorakLogger/History"))
	temp.push(previous);
...
function lootDisplay() {
	...
	(document.getElementById("number_of_rewards") as HTMLSpanElement).textContent = parseInt(JSON.parse(localStorage.getItem("ZamorakLogger/Count"))).toLocaleString("en-US");
	(document.getElementById("value_of_rewards") as HTMLSpanElement).textContent = parseInt(JSON.parse(localStorage.getItem("ZamorakLogger/Value"))).toLocaleString("en-US");
	if (parseInt(JSON.parse(localStorage.getItem("ZamorakLogger/Value"))) != 0) {
		(document.getElementById("average_of_rewards") as HTMLSpanElement).textContent = Math.round(parseInt(JSON.parse(localStorage.getItem("ZamorakLogger/Value"))) / parseInt(JSON.parse(localStorage.getItem("ZamorakLogger/Count")))).toLocaleString("en-US");
	}
	else {
		(document.getElementById("average_of_rewards") as HTMLSpanElement).textContent = "0";
	}
...
function historyInit() {
	let lsHistory = JSON.parse(localStorage.getItem("ZamorakLogger/History"))
	...
	if (lsHistory.length == 0) {
		...
		let index = parseInt(localStorage.getItem("ZamorakLogger/Count"));
		let limit = 0;
...
export function rollbackYes(id: any) {
	...
	let index = parseInt(localStorage.getItem("ZamorakLogger/Count"));
	for (let i = 0; i < parseInt(localStorage.getItem("ZamorakLogger/Count")); i++) {
...
export async function verifyInsert(event: Event) {
	...
	let curr = (parseInt(localStorage.getItem("ZamorakLogger/Count")) + 1).toString();
...
export function settingsInit() {
	...
	if (seeConsoleLogs) console.log("Setting previously set radio button for Algorithm: " + localStorage.getItem("ZamorakLogger/Algorithm") + "...");
	let temp = localStorage.getItem("ZamorakLogger/Algorithm");
	let ele = document.getElementById(temp) as HTMLInputElement;
	ele.checked = true;

	if (seeConsoleLogs) console.log("Setting previously set radio button for lagDetect: " + localStorage.getItem("ZamorakLogger/lagDetect") + "...");
	if (localStorage.getItem("ZamorakLogger/lagDetect") == "true") {
	...
	if (seeConsoleLogs) console.log("Setting previously set radio button for MultiButtonPressDetect: " + localStorage.getItem("ZamorakLogger/multiButtonPressDetect") + "...");
	if (localStorage.getItem("ZamorakLogger/multiButtonPressDetect") == "true") {
	...
	if (seeConsoleLogs) console.log("Setting previously set radio button for noMenu: " + localStorage.getItem("ZamorakLogger/noMenu") + "...");
	if (localStorage.getItem("ZamorakLogger/noMenu") == "true") {
	...
	if (seeConsoleLogs) console.log("Setting previously set radio button for hybridPrecision: " + localStorage.getItem("ZamorakLogger/hybridPrecision") + "...");
	...
	if (seeConsoleLogs) console.log("Setting previously set radio button for HistoryDisplayLimit: " + localStorage.getItem("ZamorakLogger/HistoryDisplayLimit") + "...");
...
function noMenuCheck() {
	if (localStorage.getItem("ZamorakLogger/noMenu") === "true") {
		noMenuInterval = window.setInterval(async function () {
			...
			let loc = img.findSubimage(imgs.zamorakChest);

			let rewardreader = new ZamorakReader();
			...
			if (window.alt1) {
				...
				alt1.overLayRect(a1lib.mixColor(255, 50, 50), loc[0].x + 301 - (5 * length) + (1 * comma), loc[0].y + 218, 2 + (8 * length) + (4 * comma), imgs.zamorakChest.height + 6, 60000, 2);
...
export function exporttocsv() {
	...
	let count = localStorage.getItem("ZamorakLogger/Count")
	let value = localStorage.getItem("ZamorakLogger/Value")
	...
	let filename = "ZamorakLogger CSV " + (d.getFullYear() + "-" + month.slice(-2) + "-" + day.slice(-2) + "--" + hour.slice(-2) + "-" + minute.slice(-2) + "-" + second.slice(-2)) + ".csv";
